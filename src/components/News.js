            ...
            articles.map((item) => {
              return (
                <div className="col-md-4 my-3" key={counter++}>
                  <NewsItem
                    title={item && item.title ? item.title.slice(0, 45) : "Title not available"}
                    description={item && item.description ? item.description.slice(0, 128) : "Description not available."}
